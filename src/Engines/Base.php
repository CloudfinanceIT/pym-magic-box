	...
	protected function getListenURL(string $action, array $params=[], bool $absolute=true)
	{
        if (!empty($action)) {
            ...
            if (method_exists($this, $funName)) {	
                return route("pym-magic-box.merchant-action", array_merge($params,["merchant" => $this->performer->merchant_id, "method" => Str::kebab($this->performer->method->name), "action" => $action]), $absolute);
            }
        }
		return null;
	}
